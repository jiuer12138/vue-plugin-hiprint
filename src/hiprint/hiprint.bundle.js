      ...
      return BasePrintElement.prototype.getConfigOptionsByName = function (t) {
        ...
        if (config.showPosition) {
          if (topPos.length) {
            ...
            if (config.positionUnitMM){
              topPos.text(hinnn.pt.toMm(this.options.posTop()) + (config.positionUnit ? 'mm' : ''));
            }else {
        ...
        if (config.showPosition) {
          if (leftPos.length) {
            if (config.positionUnitMM){
              leftPos.text(hinnn.pt.toMm(this.options.posLeft()) + (config.positionUnit ? 'mm' : ''));
            }else {
  ...
  n = jQuery, i = {
    ...
    refreshSizeBox: function refreshSizeBox(t, box, o) {
      ...
      if (t && t.length) {
        ...
        style = t[0].style;
        if (!style.width) {
          if (window.HIPRINT_CONFIG.sizeBoxUnitMM){
            style.width = hinnn.px.toMm(t[0].offsetWidth) + "mm";
          }else {
            style.width = t[0].offsetWidth + "pt";
          }
        }
        if (!style.height) {
          if (window.HIPRINT_CONFIG.sizeBoxUnitMM){
            style.height = hinnn.px.toMm(t[0].offsetHeight) + "mm";
          }else{
            style.height = t[0].offsetHeight + "pt";
          }
        }
        sizeBox = t.children("div[panelindex]").find(".size-box");
      }
      if (sizeBox) {
        if (window.HIPRINT_CONFIG.sizeBoxUnitMM){
          style.width=hinnn.pt.toMm(style.width.replace('pt',''))+'mm'
          style.height=hinnn.pt.toMm(style.height.replace('pt',''))+'mm'
        }
